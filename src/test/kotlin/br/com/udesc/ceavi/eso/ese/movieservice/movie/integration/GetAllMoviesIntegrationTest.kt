...
import br.com.udesc.ceavi.eso.ese.movieservice.infrastructure.movie.controller.GetAllMoviesController
...
class GetAllMoviesIntegrationTest @Autowired constructor(
    private val movieRepository: MovieRepository,
    private val getAllMoviesController: GetAllMoviesController
) {
    ...
    @BeforeEach
    fun setup() {
        movieRepository.save(MovieSchema(
            imdb = imdb,
            name = name,
    ...
    fun test() {
        assertDoesNotThrow {
            getAllMoviesController.getAllMovies().movies.let { movies ->
                for (movie in movies) {
                    assert(
                        movie.imdb == imdb
                                && movie.name == name
